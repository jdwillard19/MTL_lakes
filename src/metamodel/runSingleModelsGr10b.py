...
from pytorch_data_operations2 import buildLakeDataForRNN_manylakes_finetune2, parseMatricesFromSeqs


save_output = False
lake_ind = int(sys.argv[1])
...
ids = ids['id'].values #source ids
target_ids = [ids[lake_ind]]
...
for target_id in target_ids: 
    ...
    hypsography_target) = buildLakeDataForRNN_manylakes_finetune2(target_id, data_dir_target, seq_length, n_features,
                                       win_shift = win_shift, begin_loss_ind = begin_loss_ind, outputFullTestMatrix=True, postProcessSplits=False,
                                       sparseTen=False, realization='none', allTestSeq=True, oldFeat=False, normGE10=True)
    ...
    for source_id in ids:
        ...
        label_mats[:] = np.nan
        #save output path
        save_output_path = "../../results/transfer_learning/target_"+target_id+"/source_"+source_id+"/PGRNN_basic_normAllGr10"
    ...
    if not os.path.exists("../../results/transfer_learning/target_"+target_id+"/"):
        os.mkdir("../../results/transfer_learning/target_"+target_id+"/")
    print("../../results/transfer_learning/target_"+target_id+"/resultsPGRNNbasic_norm2_gr10_rmse")
    with open("../../results/transfer_learning/target_"+target_id+"/resultsPGRNNbasic_norm2_gr10_rmse",'w') as file:
