...
result_df = pd.read_csv("./RMSE_transfer_test_extended_glm.csv")
...
feats = ['n_obs_su', 'obs_temp_mean', 'obs_temp_std', 'obs_temp_skew',
       'obs_temp_mean_airdif', 'dif_max_depth', 'dif_surface_area',
       'dif_snow_mean_su', 'dif_ws_mean_au', 'dif_lathrop_strat',
       'perc_dif_max_depth', 'perc_dif_surface_area', 'dif_sqrt_surface_area',
       'perc_dif_sqrt_surface_area']
#declare model
...
csv = ['target_id,rmse,best_predicted,site_id']
...
for targ_ct, target_id in enumerate(test_lakes): #for each target lake
	...
	lake_df = pd.read_feather("../../metadata/diff/target_"+ target_id +"_pball_July2020.feather")
	...
	csv.append(",".join([str(target_id), str(y_act_top), str(best_predicted_rmse), str(best_predicted)]))
...
with open('pbmtl_glm_transfer_results.csv','w') as file:
    for line in csv:
        file.write(line)
        file.write('\n')
