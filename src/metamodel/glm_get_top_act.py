...
feats = ['n_obs', 'obs_depth_frac', 'obs_temp_mean', 'obs_temp_mean_airdif',
       'dif_max_depth', 'dif_surface_area', 'dif_snow_mean_su',
       'dif_ws_mean_au', 'dif_lathrop_strat', 'dif_glm_strat_perc',
       'perc_dif_max_depth', 'perc_dif_surface_area', 'dif_sqrt_surface_area',
...
# model.fit(X,y)
# dump(model, 'GLM_RMSE_GBR_pball.joblib') 
model = load('GLM_RMSE_GBR_pball.joblib') 
###############################################################
...
csv = ['target_id,best_predicted_site_id,predicted_rmse']
...
for targ_ct, target_id in enumerate(test_lakes): #for each target lake
	...
	lake_df = pd.read_feather("../../metadata/diff/target_"+ target_id +"_pball_update.feather")
	...
	top_rmse_per_lake[targ_ct,:] = train_df[train_df['target_id'] == 'nhdhr_'+target_id]['rmse'].sort_values().values[:10]
	...
	csv.append(",".join([str(target_id), str(best_predicted), str(best_predicted_rmse)]))


print(np.median(top_rmse_per_lake))
...
# with open('glm_transfer_pball_test_lakes.csv','w') as file:
#     for line in csv:
#         file.write(line)
#         file.write('\n')
