...
# profile values to loop through
lakes = [sys.argv[1]]
...
for n_prof, seed in product(n_profiles, seeds):
    ...
    hypsography) = buildLakeDataForRNN_manylakes_finetune2(lakename, data_dir, seq_length, n_features,
                                       ...
                                       sparseCustom=n_prof, randomSeed=seed) 
    ...
    for epoch in range(n_ep):
        ...
        for i, batches in enumerate(multi_loader):
            ...
            if use_gpu:
                loss_outputs = loss_outputs.cuda()
                loss_targets = loss_targets.cuda()
            loss = mse_criterion(loss_outputs[torch.from_numpy(loss_indices)], loss_targets[torch.from_numpy(loss_indices)]) + lambda1*reg1_loss + ec_lambda*unsup_loss + dc_lambda*dc_unsup_loss
            #backward
        ...
        if converged and eps_converged >= 10:
            ...
            break
    print("TRAINING COMPLETE for ", n_prof, " hid : seed ", seed)
    saveModel(lstm_net.state_dict(), optimizer.state_dict(), save_path)
