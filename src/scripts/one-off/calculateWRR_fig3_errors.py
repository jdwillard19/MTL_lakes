...
for target_id in lakenames: 
	...
	mod = ""
	pg_out0f = pd.read_feather("../manylakes2/outputs_full/"+target_id+"PGRNN_output_trial0.feather")
	pg_out0 = pg_out0f.values[:,1:]
	pg_out0 = np.array(pg_out0, dtype=np.float32)
	pg_out1f = pd.read_feather("../manylakes2/outputs_full/"+target_id+"PGRNN_output_trial1.feather")
	pg_out1 = pg_out1f.values[:,1:]
	pg_out1 = np.array(pg_out1, dtype=np.float32)
	pg_out2f = pd.read_feather("../manylakes2/outputs_full/"+target_id+"PGRNN_output_trial2.feather")
	pg_out2 = pg_out2f.values[:,1:]
	pg_out2 = np.array(pg_out2, dtype=np.float32)	
	pg_out3f = pd.read_feather("../manylakes2/outputs_full/"+target_id+"PGRNN_output_trial3.feather")
	pg_out3 = pg_out2f.values[:,1:]
	pg_out3 = np.array(pg_out3, dtype=np.float32)	
	pg_out4f = pd.read_feather("../manylakes2/outputs_full/"+target_id+"PGRNN_output_trial4.feather")
	pg_out4 = pg_out4f.values[:,1:]
	pg_out4 = np.array(pg_out4, dtype=np.float32)	
	pg_outAvgf = pd.read_feather("../manylakes2/outputs_full/"+target_id+"PGRNN_output_avgT.feather")
	pg_outAvg = pg_outAvgf.values[:,1:]
	pg_outAvg = np.array(pg_outAvg, dtype=np.float32)
	...
	outp0_dates = pg_out0f.values[:,0]
	outp1_dates = pg_out1f.values[:,0]
	outp2_dates = pg_out2f.values[:,0]
	outp3_dates = pg_out3f.values[:,0]
	outp4_dates = pg_out4f.values[:,0]
	outpA_dates = pg_outAvgf.values[:,0]
	...
	plab = plab[np.isin(plab_dates,outp0_dates)]
	plab_dates = plab_dates[np.isin(plab_dates,outp0_dates)]
	...
	pg_out0 = pg_out0[np.isin(outp0_dates,plab_dates)]
	pg_out1 = pg_out1[np.isin(outp1_dates,plab_dates)]
	pg_out2 = pg_out2[np.isin(outp2_dates,plab_dates)]
	pg_out3 = pg_out2[np.isin(outp3_dates,plab_dates)]
	pg_out4 = pg_out2[np.isin(outp4_dates,plab_dates)]
	pg_outAvg = pg_outAvg[np.isin(outpA_dates,plab_dates)]
	...
	lab = plab
	pg_out0 = pg_out0[~np.isnan(lab)]
	pg_out1 = pg_out1[~np.isnan(lab)]
	pg_out2 = pg_out2[~np.isnan(lab)]
	pg_out3 = pg_out3[~np.isnan(lab)]
	pg_out4 = pg_out4[~np.isnan(lab)]
	pg_outAvg = pg_outAvg[~np.isnan(lab)]
	...
	pg_out1 = pg_out1[~np.isnan(pg_out0)]
	pg_out2 = pg_out2[~np.isnan(pg_out0)]
	pg_outAvg = pg_outAvg[~np.isnan(pg_out0)]
	rn_out0 = rn_out0[~np.isnan(pg_out0)]
	rn_out1 = rn_out1[~np.isnan(pg_out0)]
	rn_out2 = rn_out2[~np.isnan(pg_out0)]
	# rn_outAvg = (rn_out0 + rn_out1 + rn_out2) / 3
	rn_outAvg = rn_outAvg[~np.isnan(pg_out0)]
	lab = lab[~np.isnan(pg_out0)]
	pg_out0 = pg_out0[~np.isnan(pg_out0)]
	

	pg0_rmse = str(np.sqrt(((pg_out0 - lab) ** 2).mean()))
	pg1_rmse = str(np.sqrt(((pg_out1 - lab) ** 2).mean()))
	pg2_rmse = str(np.sqrt(((pg_out2 - lab) ** 2).mean()))
	pgA_rmse = str(np.sqrt(((pg_outAvg - lab) ** 2).mean()))
	...
	csv.append(",".join([target_id, pg0_rmse, pg1_rmse, pg2_rmse, pgA_rmse, rn0_rmse, rn1_rmse, rn2_rmse, rnA_rmse]))


with open("./pgrnn_results_0927.csv",'a') as file:
	for line in csv:
		file.write(line)
		file.write('\n')