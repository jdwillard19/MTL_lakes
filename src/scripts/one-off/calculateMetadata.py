...
ids = pd.read_csv("../../../metadata/sites_moreThan10ProfilesWithGLM2.csv")
ids = ids['id'].values
...
for i, lake in enumerate(ids):
    ...
    #     continue
    meteo_path = getMeteoFileName(lake)
    nml_data = []
    with open('../../../data/raw/usgs_zips/cfg/nhdhr_'+lake+'.nml', 'r') as file:
    ...
    meteo = pd.read_csv("../../../data/raw/usgs_zips/meteo/"+meteo_path)
    ...
    # glm = pd.read_csv("../../../data/raw/usgs_zips/glm/pb0_nhdhr_"+name+"_temperatures.csv")
    ...
    # diffs = [np.abs(row.iloc[0] - row.iloc[np.where(np.isfinite(np.array(row.iloc[1:].values,dtype=np.float32)))[0][-1]+1]) for ind,row in glm.iterrows()]

    # diffs = np.array([ row.values[-9] - row.values[1] if len(row) > 8 and math.isnan(row.values[-8]) and not isinstance(row.values[-9], str)
    #                    else (row.values[-8] - row.values[1]) if len(row) > 7 and math.isnan(row.values[-7]) and not isinstance(row.values[-8], str) 
    #                    else (row.values[-7] - row.values[1]) if len(row) > 6 and math.isnan(row.values[-6]) and not isinstance(row.values[-7], str) \
    #                    else (row.values[-6] - row.values[1]) if len(row) > 5 and math.isnan(row.values[-5]) and not isinstance(row.values[-6], str)\
    #                    else (row.values[-5] - row.values[1]) if len(row) > 4 and math.isnan(row.values[-4]) and not isinstance(row.values[-5], str)\
    #                    else (row.values[-4] - row.values[1]) if len(row) > 3 and math.isnan(row.values[-3]) and not isinstance(row.values[-4], str)\
    #                    else (row.values[-3] - row.values[1]) if len(row) > 2 and math.isnan(row.values[-2]) and not isinstance(row.values[-3], str)\
    #                    else (row.values[-2] - row.values[1]) if len(row) > 1 and math.isnan(row.values[-1]) and not isinstance(row.values[-2], str)\
    #                    else 0 for ind, row in glm.iterrows()])
...
metadata.to_feather("../../../metadata/lake_metadata_2700plusWithNewGLM.feather")